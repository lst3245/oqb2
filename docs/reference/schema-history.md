...
| `question_assets.asset_type` enum widened to `QUE/ANS/SOL/WHOLE` | boot patch (`MODIFY COLUMN` when `'WHOLE'` is missing from `COLUMN_TYPE`) | `WHOLE` is IMG-only on a root QID: unsplit original used by Combine restore. Existing QUE/ANS/SOL rows unchanged. Dashboard / generator / viewer / Explain never read it. |
| `subject_prompt_notes`, `tag_corrections`; `description VARCHAR(300) NULL` on `topics`, `subtopics`, `chapters`, `subchapters` | boot patch (`create checkfirst` + `INFORMATION_SCHEMA` guard per table) | Additive. Per-subject Auto Tag instructions / correction log / taxonomy hints. See [ADR-012](../decisions/ADR-012-subject-prompt-layer-over-fine-tuning.md), [../modules/subject-ai.md](../modules/subject-ai.md). |
