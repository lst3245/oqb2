...
| 9 | `PDF_PART_BOX_JSON_CONTRACT` (format; same box vars), `PDF_PART_BOX_SYSTEM` / `PDF_PART_SOL_BOX_SYSTEM` (system; vars `json_contract`, `expected_note`), `PDF_PART_BOX_USER` (user →`PDF_PART_BOX_JSON_CONTRACT`; vars `what`, `box_pairs`, `expected_note`) | Parser `parse_part_boxes`. Pass-2 crop-relative boxes; labels `stem` / `a` / `ci` / nested `d`, `di`, `dii` (a part's own introduction is the letter alone, never `stem`). Used by PDF split-detect, the agent and the Split tool |
| 13 | `PDF_AGENT_OUTLINE_JSON_CONTRACT` (format), `PDF_AGENT_OUTLINE_SYSTEM` (system; vars `what`, `json_contract`), `PDF_AGENT_OUTLINE_USER` (user →contract; vars `page_list`, `total_pages`, `what`, `running_note`) | Parser `parse_agent_outline` → `{pages:[{page, kind}], questions:[{label, pages, marks, parts (nested), depends_prev}], paper}`. Multi-image call on captioned thumbnails. See [pdf-agent.md](pdf-agent.md) |
| 14 | `PDF_AGENT_VERIFY_JSON_CONTRACT` (format), `PDF_AGENT_VERIFY_SYSTEM` (system; vars `what`, `json_contract`), `PDF_AGENT_VERIFY_USER` (user →contract; vars `legend`, `expected`, `page_note`) | Parser `parse_agent_verify` → `{ok, issues:[{box, label, problem, fix, new_label, note}], depends_prev}`; `problem` ∈ missing/wrong_extent/overlaps/mislabelled/not_a_part/extra, `fix` ∈ relabel/drop/extend_top/extend_bottom/redetect/none; `ok` is forced false while any fix ≠ none. `page_note` is empty on a single-page question; on a continuation page it names parts that already live elsewhere so the model does not report them as missing |
...
- `build_pdf_agent_outline_system` / `build_pdf_agent_outline_user_text`, `build_pdf_agent_verify_system` / `build_pdf_agent_verify_user_text(legend, expected_labels, page_note='')`
