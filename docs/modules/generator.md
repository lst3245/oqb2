...
| GET, POST | `/generate/viewer` | login | Present mode. Accepts `question_ids[]`, `sort_config`, `sort_group_order`, `hierarchy_mode` (form or query; session fallback `viewer_question_ids`, `viewer_sort_config`). Applies `apply_multi_sort` then `resolve_render_plan`; `viewer.html` slides/drawer are **leaves** with `part`, `stem_id`, `whole_root_id` (root id when it keeps an IMG WHOLE archive, else null), `breadcrumb`, and `context` (`[{id, qid, label, kind:'stem'|'earlier'}]` — every QUE-bearing `stem`-role item `resolve_render_plan([leaf])` emits before the leaf: root stem, nested stems, earlier parts for `needs_prev_parts`). All of them load into `#stemPanel` via `viewer_asset`. Not gated on `can_generate` server-side (the dashboard button is). |
...
4. The thread sorts, splits, expands with `hierarchy.resolve_render_plan(mode=hierarchy_mode)`, builds each docx with python-docx (`create_word_document` → `(doc, doc_insertions)`), and writes to `storage.ensure_user_generated_dir(gen_file.user)` (`User/<name>/generated/`). Stems emit QUE only (no info line). Leaves get seq via `seq_owner_id`. `_plan_document_entries` tags every entry with its heading `owner` (seq owner for leaves, root id for stems) and `group_seq`; `_assign_headings` then flags the **first QUE-bearing entry per owner** with `heading_qid` / `heading_seq`, so the `N. QID` line prints once per whole question (on the stem when it has QUE, else on the first part) and parts print without a heading. The same flags gate inline and THEN-mode ANS/SOL headings (leaf-only list, flagged separately). A leaf without ANS/SOL uses the nearest ancestor once per stem run.
...
| `show_qid`, `show_qid_answer`, `show_correct_pct` (`QID [75%]`) | bool | heading printed **once per whole question** (`_assign_headings`), never repeated on parts |
...
`viewer.html` is a self-contained page (own copies of `oqbTypesetMath` / `oqbRenderMarkdownInto`; no rerender-thumbnail button). Slides and the drawer list **leaves** from `resolve_render_plan`. `loadStemQue` renders one `.stem-context-block` per entry of `question.context` (root stem, nested stems like `Q1c`, earlier parts) into `#stemPanel`, fetching each QUE in parallel; it falls back to the legacy `stem_id`/`stem_has_que` pair when `context` is absent. `applyZoom('QUE')` sizes the images in `#stemContainer` and `#wholeContainer` together with `#queContainer`, so the background follows the QUE zoom controls. The Question panel header shows a `Whole` chip (`#queWholeBtn`, `.available` only when `question.whole_root_id`) — `toggleWholeView` / `syncWholeView` fetch `viewer_asset/<root>/WHOLE` into `#wholePanel` and hide background + part while on; the preference persists across parts of the same root and resets when the next root has no archive. `loadAsset(type, qid)` and `loadAnswerAsset(qid)` fetch `/generate/api/viewer_asset/...?version_priority=` and render image parts, MD `html`, or the DOC `thumbnail_url` (polling until ready). Supports answer preference (ANS/SOL primary with fallback), zoom per panel, layouts, theatre/fullscreen, question drawer, keyboard shortcuts, and a Markup hand-off (`markupUrlFromAsset`).
...
16. **Pass `entry['heading_qid']` / `entry['heading_seq']` to `add_question_content_to_doc`, never raw `show_qid` / `entry['seq_no']`.** `seq_no` stays on every leaf for the compact MC key and `leaf_seq_nos`; the heading flags are what keep a split question to one `N. QID` line. Flag the QUE list and the answers list separately (`_assign_headings` is pure and keyed on `owner`).
17. **`viewer_asset` accepts `WHOLE` only for the Present `Whole` chip.** `viewer()` sets `whole_root_id` per slide when the root has an IMG WHOLE; `syncWholeView` swaps `#stemPanel` + `#queContainer` for `#wholePanel`. Slides, the drawer and the docx never see WHOLE.
