...
| `templates/admin_pdf_import.html` | **Run AI agent** in Step 2 (after Load PDF; does not auto-start), agent stage log, Attention panel, per-row "uses earlier parts" toggle, derived role chips, Add-part-below |
...
|---|---|---|---|
| GET | `/admin/pdf-import/agent?token&endpoint_id&parallel&debug&restart&attach&frame_snap` | `@admin_required` + token subject check (`_pdf_load_token_meta`) | Spectator SSE over a **background** job. Events `{type: info\|success\|skip\|error\|attention\|heartbeat\|done, message, stage, current?, total?}`; `stage` ∈ `outline, locate, segment, verify, done`. `done` carries `plan`, `attention`, `outline` (per-side summary), `calls`. First event is `{type:'job', job_id, attached}`. `restart=1` cancels a live run and starts over; `attach=1` (or a second open of the same URL without `restart`) joins the live run and replays its events. `frame_snap=1` is passed to `iter_detect` so located boxes snap to `meta[kind]['frames']` (exam only). Heartbeats every 15 s while a verify LLM call is in flight. |
...
## Gotchas
- An agent run **replaces `plan.json`** (it starts from `iter_detect`). Run it before manual edits, or re-run knowing edits are lost. The wizard confirms before **Run AI agent** when boxes exist (that click sends `restart=1`). The button is in Step 2 and stays disabled until Load PDF succeeds; there is no Setup-step auto-start. If the EventSource drops, the UI rejoins once with `attach=1` instead of treating the run as dead.
