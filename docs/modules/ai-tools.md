...
| POST | `/admin/questions/<id>/ai/suggest-tags` | `{versions[], fields[], endpoint_id?}` | `suggest_tags` (NO DB write) → `{success, suggestions, display, unmatched[], fields, model, confidence{field: 0..1}, reasons{field: str}}`; on parse failure `{success: false, error, raw}` with HTTP 200; `LLMError` → 502. The modal remembers the reply and posts `(suggested vs saved)` to `/admin/questions/<id>/ai/tag-corrections` when Save Tags follows (see [subject-ai.md](subject-ai.md)) |
