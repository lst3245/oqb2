...
- `TYPE` in `QUE|ANS|SOL|WHOLE`; `PART` optional int (default 1); `EXT` maps via `determine_file_format`: `png|jpg|jpeg|gif|bmp` -> IMG, `doc|docx` -> DOC, `md|markdown` -> MD, anything else -> `None` (skipped). `WHOLE` is ingested only as IMG on a root QID (`Q5`); a part path or range stem (`Q5a`, `Q23-24`) or a non-image is skipped. Smart Import still only proposes `QUE/ANS/SOL`.
