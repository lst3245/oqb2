...
Response context for the partial: `questions` (flat leaf dicts), `question_groups` (`[{stem, leaves, rows}]` — `stem` is null for standalone cards; `rows` is the ordered tree `[{kind:'substem'|'leaf', card}]` with intermediate stems emitted once before their first part), `page`, `total_pages`, `total` (**whole questions** = distinct roots), `total_parts` (non-stem rows), `all_question_ids` (leaves only), `sort_config`, `admin_subjects`. Every card carries `depth` (root 0). Each leaf may include `stem_qid` / `stem_preview`; the template uses them only when the leaf is rendered as a standalone card outside its group (`ids`/`qids` overrides), because inside a group the header already shows the background once.
...
**Pagination counts whole questions.** `paginate_by_root` buckets the sorted rows by `root(q).id` (first-seen order), slices the *root* order by `per_page`, and emits every matched part of each root contiguously on that page. So `total` / `total_pages` are whole questions (a split question = 1, a standalone = 1), a root never straddles two pages, and a topic sort that would interleave `Q5a` / `Q9` / `Q5b` shows `Q5a Q5b` together at the first sibling's position. `total_parts` (non-stem rows) is passed alongside; the partial prints `N questions · M parts` and `Select All (N questions, M parts)` only when they differ. A filter that matches only some parts still counts the question once and shows only the matched parts — unmatched siblings are **not** fetched (tags live on leaves; `hierarchy_mode=whole` at generate time covers that). `enumerate_sort_groups` (Reorder blocks) still counts rows.

After pagination, `group_for_dashboard` wraps consecutive leaves that share a root under a stem header (collapsible); `filter_questions` then expands each group into `rows`, inserting intermediate stems (`Q1c`) once before their first sub-part so nested shared intros are visible and selectable. Standalone roots stay ungrouped. Select All uses `#allQuestionIds` (leaves). A stem header / substem checkbox is an aggregate over `data-leaf-ids` (selects or clears all those leaves; shows indeterminate when only some are selected) — the stem id itself is never stored, so Select All, the counter, Set Ops and Generate all see one consistent leaf-only set. `resolve_render_plan` re-adds the stem background at render time. Topic filters match tagged **leaves**; the stem is only a header.
...
16. **A page never splits a root.** `paginate_by_root` is the only slice; do not reintroduce `sorted_questions[start:end]`. `per_page` counts roots, so a page of long ICT questions can be tall — that is the trade-off, surfaced by the `· M parts` suffix. `updateVisibleCountDisplay` counts `.question-card:not([data-stem])` so "page shows N" stays comparable with the leaf-only selection size.
