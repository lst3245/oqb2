...
| `templates/partials/question_list.html` | HTMX target fragment swapped into `#questionList`. Emits `#allQuestionIds[data-ids]` (**leaves only** across all pages). Jinja macros: `tag_badges`, `que_preview`, `action_buttons(compact)`, `leaf_card` (full card, standalone questions), `part_row` (compact part row), `substem_row` (intermediate stem such as `Q1c`). Split questions render as `.hierarchy-group` = header card + `.hierarchy-children` rows. Pagination links (`hx-get` with `hx-include="#filterForm"`). |
...
Response context for the partial: `questions` (flat leaf dicts), `question_groups` (`[{stem, leaves, rows}]` — `stem` is null for standalone cards; `rows` is the ordered tree `[{kind:'substem'|'leaf', card}]` with intermediate stems emitted once before their first part), `page`, `total_pages`, `total` (**whole questions** = lettered roots + standalone + each range leaf), `total_parts` (non-stem rows), `all_question_ids` (leaves only), `sort_config`, `admin_subjects`. Every card carries `depth` (root 0). Each leaf may include `stem_qid` / `stem_preview`; the template uses them only when the leaf is rendered as a standalone card outside its group (`ids`/`qids` overrides), because inside a group the header already shows the background once.
...
- `turns` is the conversation AFTER the server-built initial turn; the server rebuilds turn 1 (system `EXPLAIN_SYSTEM` + labelled QUE/SOL context via `_explain_slot_context`, plus ancestor QUE images labelled "Shared background" when the row is a part, earlier siblings when `needs_prev_parts`, and — when the row is a stem — every descendant part's QUE/SOL labelled `PART (a)`, `PART (c)(i)` …) on every call, so source images are never uploaded by the browser. Only the last 20 turns are kept; text is capped at 8000 chars per turn; empty turns are dropped.
...
**Pagination counts whole questions.** `paginate_by_root` buckets the sorted rows by `root(q).id` (first-seen order). A lettered-part tree (Q5 / Q5a) is **one** question; a range stem (Q16-17: shared MC data, independent paper numbers) is **one question per matched leaf**; a standalone is one. Buckets pack into pages by that weight without splitting a root — a range that does not fit the remaining budget starts the next page (an overweight bucket still gets a page of its own). Topic sorts that would interleave `Q5a` / `Q9` / `Q5b` snap the parts together at the first sibling. `total_parts` (non-stem rows) is passed alongside; the partial prints `N questions · M parts` and `Select All (N questions, M parts)` only when they differ. A filter that matches only some parts of a lettered question still counts it once and shows only the matched parts — unmatched siblings are **not** fetched (tags live on leaves; `hierarchy_mode=whole` at generate time covers that). `enumerate_sort_groups` (Reorder blocks) still counts rows. Range headers show `N questions share this data` instead of `N parts`.

After pagination, `group_for_dashboard` wraps consecutive leaves that share a root under a stem header (collapsible); `filter_questions` then expands each group into `rows`, inserting intermediate stems (`Q1c`) once before their first sub-part so nested shared intros are visible and selectable. Lettered part rows show `(a)`; range-stem leaves have no `part` and `part_row` omits the letter (never `(None)`). Standalone roots stay ungrouped. Select All uses `#allQuestionIds` (leaves). A stem header / substem checkbox is an aggregate over `data-leaf-ids` (selects or clears all those leaves; shows indeterminate when only some are selected) — the stem id itself is never stored, so Select All, the counter, Set Ops and Generate all see one consistent leaf-only set. `resolve_render_plan` re-adds the stem background at render time. Topic filters match tagged **leaves**; the stem is only a header.
...
12. **Jinja prints `None` as the text `None`.** A missing `part` on a range-stem leaf must be gated (`{% if part_letter %}`) — `({{ question.part }})` becomes `(None)`.
...
14. **`#allQuestionIds` must stay leaves-only.** Adding stem ids there would make Select All double-count (stem + parts). Stem header **and substem-row** checkboxes are a separate `name="stem_checkbox"` and are **aggregates, not selection members**: `selectedQuestions` holds leaf ids only. Each stem card carries `data-leaf-ids` (every matched descendant leaf in the whole result set, all pages); ticking it calls `setLeafSelected` for each id, and `refreshStemCheckboxes()` (run from `updateSelectAllCheckbox`) re-derives checked / indeterminate / `.partially-selected` from the leaves. `initializeSelectionState` migrates a legacy stem id found in the stored selection into its leaf ids. `toggleStemGroup` relies on `.stem-header-card` being the immediate previous sibling of `.hierarchy-children` inside `.hierarchy-group`.
15. **Explain on a stem sends the whole question.** `explain_question` detects `is_stem(question)` and appends every descendant's QUE (and SOL) labelled `PART (c)(i)` relative to the stem, after the stem's own text; a leaf flagged `needs_prev_parts` also gets its earlier siblings as "Earlier part (i)" context. Image count therefore scales with the part count.
16. **A page never splits a root.** `paginate_by_root` is the only slice; do not reintroduce `sorted_questions[start:end]`. `per_page` is a question-weight budget (lettered root = 1, range leaf = 1). A page of long ICT questions can be tall — that is the trade-off, surfaced by the `· M parts` suffix. `updateVisibleCountDisplay` counts `.question-card:not([data-stem])` so "page shows N" stays comparable with the leaf-only selection size.
17. **WHOLE on the group header is view-only.** Stem cards carry `has_whole` / `whole_asset_ids` (best version, IMG only); the header shows a `Whole` button (`toggleStemWhole`) that swaps `.stem-shared-preview` for `.stem-whole-preview` (images via `get_asset_preview`, click → `_explainShowImageOverlay`). Nothing else on the dashboard reads WHOLE; the typed `/preview/<asset_type>` route still rejects it. Manage the archive in the Edit modal → Assets on the root.
