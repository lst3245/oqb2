...
Response context for the partial: `questions` (flat leaf dicts), `question_groups` (`[{stem, leaves, rows}]` — `stem` is null for standalone cards; `rows` is the ordered tree `[{kind:'substem'|'leaf', card}]` with intermediate stems emitted once before their first part), `page`, `total_pages`, `total` (leaf count), `all_question_ids` (leaves only), `sort_config`, `admin_subjects`. Every card carries `depth` (root 0). Each leaf may include `stem_qid` / `stem_preview`; the template uses them only when the leaf is rendered as a standalone card outside its group (`ids`/`qids` overrides), because inside a group the header already shows the background once.
