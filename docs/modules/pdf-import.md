...
| File | Role |
|---|---|
| `app/pdf_import.py` | `parse_paper_prefix`, `parse_page_range`, `guess_paper_name`, cancel registry, staging helpers, `rasterize_pdf`, `stage`, `DETECT_METHODS`, `detect_page`, `detect_parts`, `crop_page`, `pages_to_pdf_bytes`, `export_zip_bytes`, `coerce_plan_item`, `sanitize_plan`, `plan_item_label`, `map_crop_box_to_page`, `full_width_child_box`, `whole_source_crops`, `pick_pass1_parent_box`, `split_question_png`, `map_page_box_to_stitch`, `merge_part_boxes_by_label`, `_call_with_llm_retry`, `iter_detect`, `iter_split_detect`, `detect_single_page`, `_stitch_continuations`, `_group_plan`, `iter_commit`; page-frame: `detect_frames`, `consolidate_frames`, `ensure_frames`, `set_manual_frame`, `frame_x_extent`, `snap_boxes_to_frames`, `FrameCropper` |
| `app/pdf_layout.py` | Classical-CV helpers (NumPy): `deskew_image`, `refine_box`, `segment_page`, `detect_page_frame`, `load_gray`, `numpy_available` |
...
| `templates/admin_pdf_import.html` | 3-step wizard: Step 1 Pages field + Load PDF; Step 2 mounts one `OQBBboxEditor` per page card (`card._ed`), `startSplitDetect`, **Run AI agent** (exam, after load; does not auto-start), Re-split / Unsplit, label-aware renumber, **Snap boxes to page frame** |
| `static/js/bbox_editor.js` | Shared `OQBBboxEditor` global. Host markup: `.pdf-page-wrap` > `.pdf-page-img` + `.pdf-overlay`. Boxes are fractional `[x1,y1,x2,y2]`; the host owns the item model and persistence. See [../frontend/conventions.md](../frontend/conventions.md#bounding-box-editor-oqbbboxeditor) |
| `static/css/bbox_editor.css` | Overlay, 8-handle resize, magnifier, crop preview, dashed green page-frame guide |
| `tests/test_page_frame.py` | Pure-logic: `detect_page_frame` synthetics, `consolidate_frames`, `frame_x_extent`, `snap_boxes_to_frames`, `crop_page` `trim_axis`, `FrameCropper.is_snapped` (no live DB) |
...
|---|---|---|---|
| GET | `/admin/pdf-import` | admin | Page. Passes `subjects`, `versions`, `endpoints` (vision-capable, enabled, with `kind` / `max_concurrency`), `ai_enabled`, `raster_width`, `deskew_default`, `trim_white_default`, `uniform_width_default`, `frame_snap_default`, `frame_inset_frac`, `default_method`, `default_endpoint_id`, `pdf_source_path`, `pdf_source_available` |
| POST | `/admin/pdf-import/stage` | admin (+ subject admin for exam) | Multipart. `mode` (`exam`\|`generic`, default exam). Exam: `paper` (prefix `SUBJ_SOURCE_YEAR_PAPER`, validated by `parse_paper_prefix`; subject must exist; 403 if not subject admin), optional `que_version` / `sol_version` (legacy `version` applies to both; invalid → ENO / EN). Files: `que_pdf` / `sol_pdf` uploads, or `que_server_path` (+ `que_server_root`) / `sol_server_path` (+ `sol_server_root`); at least one side required; an upload wins over a server pick for the same side. Optional `page_range` (1-based source-PDF spec `1-8` / `3,5,12-14`; blank = all pages; applied **before** A3 split; same spec on QUE and SOL; junk or no matching pages → 400). Pre-processing: `deskew` (default on), `pre_rotate` (90/180/270), `split_mode` (`none`\|`simple`\|`mode1`\|`mode2`), `mode1_pages_per_student`, `f_deskew`, `f_brightness`, `f_contrast`, `f_sharpen`, `f_grayscale`, `f_bw`, `f_bw_threshold` (default 160). Returns `{token, mode, custom_prompt, instruction, subject, source, year, paper, que_version, sol_version, version, deskew, page_range, split_parts_default, que: {filename, pages: [{index, width, height, frame}]}, sol: {...}}` (`frame` is `{box, source}` or `null`, aligned with `meta[kind]['frames']`). Does not require a vision endpoint or `AI_TOOLS_ENABLED` |
...
| GET | `/admin/pdf-import/detect` | admin (token-scoped) | SSE. Params `token`, `endpoint_id` (≤ 0 → `_pdf_default_endpoint`), `method` (`llm`\|`refine`\|`segment`), `parallel` (1/0), `debug` (1/0), `instruction` (≤ 2000 chars), `custom_prompt` (1/0), `frame_snap` (1/0; exam only — after `_stitch_continuations`, `snap_boxes_to_frames` so the saved plan matches the review UI). Generic mode requires `instruction` and stores it in `meta`; exam + `custom_prompt=1` requires `instruction` and sets `meta.custom_prompt = True`; otherwise clears both. Streams `iter_detect` and accumulates `plan.json`. Errors before streaming are returned via `_pdf_sse_error` (one `error` + `done` event) |
| GET | `/admin/pdf-import/split-detect` | admin (token-scoped) | SSE pass-2. Params `token`, `kind` (`que`\|`sol`\|`both`), `endpoint_id`, `debug`, `parallel` (cloud only), optional `labels` csv to re-split those parents only (a nested stem such as `4d` can be re-split by its own label; a parent spanning several pages is re-split as a whole), `frame_snap` (1/0 → `apply_frame_snap` before every `save_plan`). Exam sessions only. Streams `iter_split_detect`; `done` carries the replaced `plan`. Generic → SSE error |
| GET | `/admin/pdf-import/agent` | admin (token-scoped) | Spectator SSE over a background agent job (`restart`, `attach`, `frame_snap` query flags). `frame_snap` is threaded through `start_agent_job` / `iter_agent` into `iter_detect`. See [pdf-agent.md](pdf-agent.md) |
...
| POST | `/admin/pdf-import/redo-page` | admin (token-scoped) | `{token, kind, index, endpoint_id?, method?, debug?, instruction?, custom_prompt?, frame_snap?}`. Overrides are persisted to `meta` for the re-run; blank `instruction` falls back to `meta.instruction`. `frame_snap: true` (exam) snaps the returned boxes to that page's frame. → `{boxes, raw?}` (`raw` clipped to 6000 chars) |
...
| POST | `/admin/pdf-import/export-zip` | admin (token-scoped) | `{token, kind (default que), items: [{page, label, box}]}` → ZIP of PNG crops (`export_zip_bytes(..., inset_frac, normalise)`, `trim_white=False`, `pad_frac = PDF_IMPORT_CROP_PAD_PCT/100`). Frame-snapped boxes still go through `FrameCropper` (vertical trim is a no-op because trim is off; width normalise still applies). Generic Extraction's "Download all as ZIP"; no DB writes |
...
- **Step 1 — Setup**: Task toggle (`exam` / `generic`), Paper name, QUE / SOL PDFs (upload or server pick via the unified selector), "Auto-deskew scans" + collapsible "Pre-process scans" (`pre_rotate`, `split_mode`, `mode1_pages_per_student`, `f_*`), optional **Pages** (1-based source-PDF range, before A3 split), **Load PDF** (`POST /stage`). Exam staging defaults versions to ENO / EN in `meta`; the real import versions come from Step 3. There is no agent checkbox here — Load PDF only rasterises.
- **Step 2 — Bounding boxes**: endpoint, method, custom prompt + instruction, **Split questions into parts** (exam; default from `split_parts_default`), **Snap boxes to page frame** (`frameSnapChk`, default `PDF_IMPORT_FRAME_SNAP_DEFAULT`, ON), uniform width (legacy; default OFF), debug. Detection is optional — after Load PDF the page skeleton appears immediately for manual Add box / drag / resize via one `OQBBboxEditor` per page card (`card._ed`). **Run AI agent** (exam only; enabled after load) outlines / locates / splits / verifies and **replaces** the plan; it never auto-starts on load. When detection runs, page cards render up-front with a "detecting…" badge and fill in as each `page` event arrives (`fillPageCard`). If Split is ticked, pass 1 finishing (or ticking the box later) auto-runs `startSplitDetect`. Per-page: drag move, 8-handle resize, Add box (draw mode), edit qno / label (`3` / `3a` / `23-24`), Delete key, Re-run with per-page method / endpoint (`/redo-page`), per-question **Re-split** / **Unsplit**, click crop to enlarge (`showCropPreview`), **Frame** button + dashed green guide, Download processed PDF per side. Grey = stem, amber = part. While frame snap is on, detect/agent are called with `frame_snap=1`.
...
The IMG Split-into-parts page reuses pass 2 via `split_question_png` (default `find_parent=False`) on the **staged stitch**: `detect_parts` + `full_width_child_box` with one `LLMError` retry. If QUE has several images (Combine-reconstructed part crops, a multi-page WHOLE, …) they are stacked first; a single image is used as-is. Progress is SSE (`GET /admin/questions/<id>/split/detect`). `find_parent=True` adds pass 1 for a full exam-page scan only. Do not run `detect_parts` on each source PNG as if it were a paper page, and do not run `detect_page` on an already-cropped library QUE.

### Multi-page questions

The model sees one page at a time. A question tail spilling onto the next page is reported as a page-top region flagged `continues_prev` (the page above flagged `continues_next`). `iter_detect` keeps these transiently as `_cp` / `_cn` on plan items; `_finalize_detect` runs once after ALL pages are collected: for custom-prompt runs it numbers 1..N per side in reading order `(page, box top-Y)`; for standard runs it calls `_stitch_continuations(plan)`, which walks each side in reading order and lets a page-top continuation inherit the qno of the box before it (only across a page boundary; chains resolve because processing is ordered). On a **frameless** page the continuation also inherits the head box's **x1/x2** (the model otherwise boxes only the indented body). On a **framed** page with `frame_snap=1`, `snap_boxes_to_frames` runs after stitch so the per-page frame wins over that inherited x — see [Horizontal extent (x1/x2) chain](#horizontal-extent-x1x2-chain). This is independent of the order parallel detection returns pages. The flags are stripped before `plan.json` is written. The review UI mirrors it with `stitchContinuations()` (in `finishDetect` and after per-page re-run). At commit, same-`qno` parts become one multi-part IMG asset (`stitch=False`).
...
### Horizontal extent (x1/x2) chain

DSE answer books print a rectangle on every page. Scans drift left/right between pages (odd/even booklet pages differ — e.g. left rail at 0.052 vs 0.087 of page width on ICT 2023 P1B), and the LLM boxes only the indented body on continuation pages, so per-question x-extents never lined up. "Uniform width" (widest box, left-anchored to each box's own `x1`) could not fix that. The printed frame is the per-page fact; x is resolved in this order:

1. **LLM box** — `detect_page` / pass-2 `detect_parts`. `_DEFAULT_PDF_QUE_BOX_SYSTEM` tells the model the left edge must be the numbered margin / inner frame edge, not the indented body.
2. **Stitch inherit (frameless only)** — `_stitch_continuations` copies the head question's `x1/x2` onto a continuation. Skipped in effect when the next step runs.
3. **Frame snap** — `apply_frame_snap(plan, meta, config)` is the single convergence point for every detection path: `iter_detect` (`/detect`, agent locate), `iter_split_detect` (`/split-detect`, agent split + repair rounds) and `/redo-page` all take `frame_snap` and call it **after** stitch (not in generic mode). The UI sends the flag from `frameSnapChk` on all three, and its own `stitchContinuations()` leaves x alone on framed pages, so a Re-run / Re-split page can never un-snap a continuation. `snap_boxes_to_frames` does the per-side work. `frame_x_extent(frame, inset_frac)` → `(x1, x2)` = rails inset by `PDF_IMPORT_FRAME_INSET_PCT/100`; y is untouched. The saved plan then matches the review UI.
4. **Review uniform width (frameless only)** — legacy `uniformWidthChk`. Framed pages ignore it while frame snap is on.
5. **Commit `FrameCropper`** — a box is "snapped" when its `x1/x2` sit within `tol=0.006` of the frame's inner extent. Snapped crops use `crop_page(..., trim_axis='vertical')` (white-trim never re-narrows them) and, when `PDF_IMPORT_FRAME_NORMALISE_WIDTH` is on, are resampled (LANCZOS, ≤15% change) to the paper's median frame pixel width. Hand-widened boxes are cropped exactly as before. `iter_commit`, the `whole_source_crops` path, and `export_zip_bytes(..., inset_frac, normalise)` all go through it.

Detection and storage of the frame itself:

- `pdf_layout.detect_page_frame(gray)` (NumPy-only): column/row scores = minimum dark fraction over 3 bands of the page (a full-height rule scores, text does not); thin runs only; a vertical rail is accepted only if a horizontal rule starts/ends at it (perpendicular partner) — rejects scanner shadows. Returns `{left, top, right, bottom}` fractions (inner edges), `None` per rail if unconfirmed.
- `pdf_import.detect_frames(kind_dir, pages)` runs it per staged page at **stage time**. `consolidate_frames(raw)` consolidates per paper: paper width = median of pages with both rails; a page missing one rail gets it from the other + width; a page with none inherits same-parity page medians, else overall median; if <40% of pages show a frame the paper is frameless (all `None`). Result: `meta[kind]['frames']` aligned with `pages`, each entry `{'box':[x1,y1,x2,y2], 'source':'detected'|'inferred'|'manual'}` or `null`.
- `ensure_frames(token, meta)` lazily computes and saves frames for staging dirs created before this existed (called by detect, commit, export, `set_manual_frame`).

See [ADR-011](../decisions/ADR-011-page-frame-crop-anchor.md).
...
- Staging dir: `<SYSTEM_PATH or OUTPUT_PATH>/.pdf_import/<token>/` holding `meta.json` (`mode`, `subject` / `source` / `year` / `paper`, `que_version` / `sol_version` / `version`, `custom_prompt`, `instruction`, `deskew`, `pre_rotate`, `split_mode`, `mode1_pages_per_student`, `filters`, `page_range`, `created_at`, per-kind `{filename, pages: [{index, filename, width, height}], frames: [{box, source}|null, ...]}` — `frames` aligned with `pages`), `plan.json`, and `que/` / `sol/` (`source.pdf` + `page_NNNN.png`, 1-based). `token = uuid4().hex`, validated by `_TOKEN_RE = ^[0-9a-f]{8,40}$` before any filesystem join. `cleanup_old()` (called on every `stage`) purges dirs older than 6 h; Discard removes one immediately. `save_meta` writes atomically via `.tmp` + `os.replace`. `ensure_frames` fills a missing / length-mismatched `frames` list on old tokens.
- `rasterize_pdf(pdf_path, out_dir, width_px, deskew, pre_rotate, split_mode, filters, workers, mode1_pages_per_student, page_range)` delegates to `app/pdf_tools.py`: `split_descriptors` → optional `parse_page_range` filter on **source** page indices → per-fragment op chain (`build_op_chain(pre_rotate, frag_ops, filters)`) → `rasterize_page` + `apply_ops`. The legacy `deskew` flag is merged into `filters['deskew']`. Splitting yields more staged PNGs, renumbered contiguously; downstream per-page detection is unaffected (Mode 1 drops padding slots such as page 8 of a 7-page booklet). `page_range` is applied before A3 split, so `1-4` keeps PDF pages 1–4 (both halves if split is on). Out-of-range numbers on a shorter PDF are skipped; a spec that matches nothing raises `ValueError` (stage → 400).
...
`crop_page(png_path, box, pad_frac, trim_white, whiteness_threshold, min_px=8, trim_axis='both')`: pads by `pad_frac` (= `PDF_IMPORT_CROP_PAD_PCT / 100`, the "safety margin"), crops, raises `ValueError('degenerate crop box')` below `min_px`; when `trim_white` is on, tightens to non-white content (threshold `THUMBNAIL_WHITENESS_THRESHOLD`, keeps an 8 px border; content is never removed, so SOL side-notes survive). `trim_axis='vertical'` trims only top/bottom — used for frame-snapped boxes so white-trim cannot re-indent a continuation. `trim_white` is a per-run toggle: checkbox default = `PDF_IMPORT_TRIM_WHITE_DEFAULT` (OFF) → `&trim_white=` on the commit URL → `iter_commit(..., trim_white=)`. OFF = the crop respects the selected box exactly (plus safety margin); ON = tighten to content. Commit and ZIP go through `class FrameCropper(meta, inset_frac, pad_frac, trim_white, whiteness, normalise, tol=0.006)`: snapped boxes use vertical trim + optional width normalise; hand-widened boxes crop as before. `export_zip_bytes` always uses `trim_white=False` but still constructs a `FrameCropper` for width normalise.
...
- **Snap boxes to page frame** (`frameSnapChk`, exam; default = `PDF_IMPORT_FRAME_SNAP_DEFAULT`, ON): boxes on framed pages take `x1/x2` from the dashed green guide; the wrap gets `x-locked` (horizontal handles hidden); drawn boxes snap; detect/agent send `frame_snap=1`. Dragging a green rail POSTs `/admin/pdf-import/frame` and re-snaps that page. Each page card has a **Frame** button toggling "No frame" / "Add frame" (seeds from same-parity pages) and a badge `frame` / `frame (inferred)` / `frame (manual)`.
- **Uniform width per side** (`uniformWidthChk`, exam + generic; **legacy**): lock every box on a side to the widest box's width. Default = `PDF_IMPORT_UNIFORM_WIDTH_DEFAULT` (OFF). Pages with a frame ignore it while frame snap is on; it still applies to frameless pages. Width grows only (monotonic) and is recomputed non-lossily after each streamed page (`enableUniformWidth(kind)` — boxes are pushed at natural width, never snapped down first) and again on re-run / finish. `conformWidth` is left-anchored (only the right edge moves). Resizing one box's width re-broadcasts via `applyUniformWidthToAll`; moving does not. Untick to fine-tune (`uniformWidth` reset to `null`).
- **Shared overlay editor**: never reimplement box overlays or a canvas painter. Mount `OQBBboxEditor`; mutate boxes through the host model then call `ed.restyle(item)`. See [../frontend/conventions.md](../frontend/conventions.md#bounding-box-editor-oqbbboxeditor).
- **Live magnifier / crop preview**: owned by `OQBBboxEditor` (`.pdf-magnifier`, `showCropPreview`). Hosts do not draw their own magnifier.
...
| `PDF_IMPORT_UNIFORM_WIDTH_DEFAULT` | bool | Initial state of legacy "Uniform width per side" (default OFF). Framed pages ignore it while frame snap is on |
| `PDF_IMPORT_FRAME_SNAP_DEFAULT` | bool | Initial state of "Snap boxes to page frame" (default ON) |
| `PDF_IMPORT_FRAME_INSET_PCT` | float 0–5 | Inset inside detected frame rails, % of page width (default 0.5). `frame_x_extent` / `snap_boxes_to_frames` / `FrameCropper` |
| `PDF_IMPORT_FRAME_NORMALISE_WIDTH` | bool | Resample frame-snapped crops to the paper's median frame pixel width (default ON; LANCZOS, ≤15% change) |
...
7. NumPy is optional: `refine` / `segment` / deskew / `detect_page_frame` need it. Detect fails fast with one SSE error; deskew silently warns and skips; missing NumPy yields all-`None` frames and the rest of the pipeline behaves as before. `segment` assumes a single column — dense MC or note-heavy SOL pages can over- or under-merge; per-page re-run with `llm` is the escape hatch.
8. Never share a `fitz.Document` across threads; each raster worker opens its own.
9. `_finalize_detect` runs after all pages — custom-prompt numbering and continuation stitching are deterministic regardless of completion order; the old per-page numbering was order-sensitive. Re-detecting a single page (`/redo-page`) replaces that page's boxes and does not re-stitch server-side; the UI's `stitchContinuations()` handles it, then `applyWidthPolicy()` — **in that order** (stitch may copy the head page's x; the frame must win afterwards). An earlier build had them reversed and Re-run produced un-snapped continuations.
10. **Re-split page** (per-card button) collects the *root* labels (`rootLabelOf`: `source_label`, else the numeric prefix) of every box on that page and calls `/split-detect?labels=…`. The server re-splits each root as a whole via `original_group_parts`, so a question that continues from/onto another page is re-cut across all its pages — this page's parts are never re-cut in isolation, which would break the cross-page link.
...
19. The AI agent run (`/agent`) starts from `iter_detect` and therefore **replaces** the plan; manual edits made beforehand are lost (the wizard confirms first). The agent lives in Step 2 (**Run AI agent** after Load PDF); it does not auto-start.
20. `page_range` is 1-based source-PDF pages, shared by QUE and SOL, applied before A3 split. Junk or a spec that matches no pages is a 400, not a 500.
21. IMG Split-into-parts (`question_split.iter_detect_boxes`) stitches multi-image QUE first, then SSE pass 2 (`detect_parts` + `full_width_child_box`, one retry on `LLMError`) **once on the stitch**. Do not send each part-crop as its own page. Do not run pass 1 (`detect_page`) on an already-cropped question unless `find_parent=1`. The split page mounts `OQBBboxEditor` on the stitch — do not reintroduce a `<canvas>` painter.
22. `trim_white` would re-indent continuation crops (the body is narrower than the printed frame). `FrameCropper` trims **vertically only** for snapped boxes (`trim_axis='vertical'`).
23. Frames are detected at **stage time**; old tokens get them lazily via `ensure_frames` (detect, commit, export, `set_manual_frame`).
24. Odd/even booklet pages have different scan offsets — `consolidate_frames` uses same-parity medians before the overall median. If fewer than 40% of pages show a frame the paper is frameless.
25. Frame snap wins over uniform width. Uniform width is legacy (default OFF) and still applies only to frameless pages while snap is on.

## Related

- [pdf-agent.md](pdf-agent.md) — the AI agent layer that drives these tools with outline hints and an attention list (ADR-010).
- [question-hierarchy.md](question-hierarchy.md) — tree model; this wizard commits `ensure_question` tokens including parts. IMG Split-into-parts reuses `split_question_png`.
...
- [../core/06-system-settings.md](../core/06-system-settings.md), [../decisions/ADR-004-db-backed-settings-with-env-bootstrap.md](../decisions/ADR-004-db-backed-settings-with-env-bootstrap.md), [../decisions/ADR-011-page-frame-crop-anchor.md](../decisions/ADR-011-page-frame-crop-anchor.md)
- [../frontend/conventions.md](../frontend/conventions.md#bounding-box-editor-oqbbboxeditor) — `OQBBboxEditor`
