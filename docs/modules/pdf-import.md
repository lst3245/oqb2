...
| File | Role |
|---|---|
| `app/pdf_import.py` | `parse_paper_prefix`, `guess_paper_name`, cancel registry (`_PDF_CANCEL`, `new_job`, `cancel_job`, `finish_job`), staging helpers (`staging_root`, `token_dir`, `page_png_path`, `load_meta`/`save_meta`, `load_plan`/`save_plan`, `discard`, `cleanup_old`), `rasterize_pdf`, `stage`, `DETECT_METHODS`, `detect_page`, `crop_page`, `pages_to_pdf_bytes`, `export_zip_bytes`, `_safe_filename`, `_check_method_available`, `iter_detect`, `detect_single_page`, `_stitch_continuations`, `_group_plan`, `iter_commit` |
...
| `app/ai_prompts.py` | `PDF_QUE_BOX_SYSTEM`, `PDF_SOL_BOX_SYSTEM`, `PDF_BOX_JSON_CONTRACT`, `PDF_BOX_USER`, `build_pdf_box_system` / `build_pdf_box_user_text`, `pdf_box_order_vars`, `parse_question_boxes`; `PDF_GENERIC_BOX_*`, `build_pdf_generic_system` / `build_pdf_generic_user_text`, `parse_generic_boxes`; `PDF_ANCHOR_*`, `build_pdf_anchor_*`, `parse_question_anchors`; `PDF_PAPER_NAME_*`, `build_pdf_paper_name_*`, `parse_paper_name`. See [ai-prompts.md](ai-prompts.md) |
...
| `templates/admin_pdf_import.html` | 3-step wizard UI: `updateParallelUI`, `fillPageCard`, `stitchContinuations`, `renumberAllSequential`, `renumberAfterInsert`, `renumberAfterDelete`, `updateOrderWarning`, `enableUniformWidth`, `conformWidth`, `applyUniformWidthToAll`, `updateDragMagnifier`, `showCropPreview`, `startCommit` |
...
|---|---|
| `questions` (`Question`) | `iter_commit` find-or-creates by `qid = {subject}_{source}_{year}_{paper}_Q{qno}` with `subject`, `source`, `year`, `paper`, `qno`, `q_type` (via `ingestor.determine_question_type`). `section` / `level` stay null (tagged later) |
| `question_assets` (`QuestionAsset`) | `replace_img_assets(question, atype, version, imgs, stitch=False, source_path)` deletes existing IMG rows for the slot and writes new `file_format='IMG'` parts (one per plan part) |
...
| `subjects` (`Subject`) | Existence check on stage |
...
| POST | `/admin/pdf-import/plan` | admin (token-scoped) | `{token, plan: {que: [...], sol: [...]}}`. Sanitises every item to `{page:int, qno:int|None, box:[4 floats]}` (qno = first integer found in the string) and overwrites `plan.json` → `{success, counts: {que, sol}}` |
...
- **Step 2 — Bounding boxes**: endpoint, method, custom prompt + instruction, uniform width, debug. Detection is optional — after Load PDF the page skeleton appears immediately for manual Add box / drag / resize. When detection runs, page cards render up-front with a "detecting…" badge and fill in as each `page` event arrives (`fillPageCard`). Per-page: drag move, handle resize, Add box, edit qno / label, delete, Re-run with per-page method / endpoint (`/redo-page`), click crop to enlarge (`showCropPreview`), Download processed PDF per side.
...
`_group_plan(plan)` groups by `(kind, qno)` ordered by `(page, top-Y)`; items with `qno is None` or a malformed box are skipped. For each group: find-or-create `Question` (QID `{subj}_{source}_{year}_{paper}_Q{qno}`, committed immediately so a later crop failure does not lose it); if `slot_has_img(question.id, atype, version)` and not `overwrite` → `skip`; else `crop_page` every part and `replace_img_assets(question, atype, version, imgs, stitch=False, source_path)` (atomic; deletes existing IMG rows first — that IS the overwrite path; fires the DOC-thumbnail lifecycle). Exceptions roll back and emit `error`.
...
- **Auto-renumber** (exam): reading order within a side = `(page, box top-Y)`. Add (`finishDraw` → `renumberAfterInsert`): new qno = predecessor + 1 (or 1), every later box +1. Delete (`renumberAfterDelete`): every later box −1 (clamped ≥ 1). `refreshNumbersUI` rewrites labels + inputs. Re-detecting a page replaces its boxes with the model's numbers and does not cascade; manual qno edits and move / resize never cascade.
- **Out-of-order warning** (`updateOrderWarning`, exam only): red banner when a side's qnos are not ascending in reading order (a repeat = spanning question is fine; a drop is flagged with Q# / pages).
...
- `detect` and `commit` are GET SSE streams (EventSource) run inside a pushed `app.app_context()` with `Cache-Control: no-cache`, `X-Accel-Buffering: no`. The dev server must run threaded.
...
1. Detection, redo-page, commit and guess-paper are gated on `AI_TOOLS_ENABLED` and need a vision-capable `LLMConfig` (SSE routes surface this as a single `error` + `done`). Staging, page images, plan save, processed PDF, ZIP export and discard work without AI.
...
6. `replace_img_assets` deletes the slot's existing IMG rows before writing — that is the overwrite path. The only guard is the `slot_has_img` skip when `overwrite` is off.
...
10. Continuation flags `_cp` / `_cn` are transient; `plan.json` and `/plan` payloads are `{page, qno, box}` only (plus `label` in generic mode).
...
16. Plan sanitisation extracts the first integer from any `qno` string; non-numeric labels become `None` and are skipped at commit ("Make sure every region has a question number").
