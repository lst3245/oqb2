...
| GET | `/admin/pdf-import/split-detect` | admin (token-scoped) | SSE pass-2. Params `token`, `kind` (`que`\|`sol`\|`both`), `endpoint_id`, `debug`, `parallel` (cloud only), optional `labels` csv to re-split those parents only. Exam sessions only. Streams `iter_split_detect`; `done` carries the replaced `plan`. Generic → SSE error |
...
| POST | `/admin/pdf-import/plan` | admin (token-scoped) | `{token, plan: {que: [...], sol: [...]}}`. `sanitize_plan` keeps `{page, qno, label, box, role?, source_label?, source_page?, source_box?}`. Exam labels are QNO tokens (`5`, `5a`, `23-24`); generic keeps free-text labels. Overwrites `plan.json` → `{success, counts: {que, sol}}` |
...
4. SOL uses QUE labels as `expected_labels`. Unmatched lettered SOL is skipped at commit; leftover whole-question SOL attaches to the stem.
5. `labels` csv re-splits one parent (Re-split button). Unsplit merges children back using `source_box` / `source_page`.
...
10. Continuation flags `_cp` / `_cn` are transient; exam `plan.json` and `/plan` payloads are `{page, qno, label, box, role?, source_*}`. Generic stays `{page, label, box}`.
