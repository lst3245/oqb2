...
| `app/dashboard.py` | Leaves-only filter (unless `qids`/`ids`); `paginate_by_root` (whole-question pages) then `group_for_dashboard`, then builds `rows` (substem + leaf, with `depth`) per group; stem cards carry `whole_asset_ids` for the header `Whole` toggle; Explain prepends ancestor QUE (never WHOLE) |
...
- **One ID heading per whole question (docx).** `generator._assign_headings` flags the **first QUE-bearing entry of each heading group** (`owner` = seq owner for leaves, root id for stems) with `heading_qid` / `heading_seq`; only that entry prints `N. QID`. So a split question prints `1. ECON_..._Q3` on the stem (or on `(a)` when the stem has no QUE) and parts `(a)(b)(c)` print with no heading — the crop already carries the letter. Nested stems, `needs_prev_parts` earlier siblings and later parts never get a heading. A range preamble (`Q23-24`) still prints its QID-only heading and each MC leaf numbers itself. The answers section is flagged over its own leaf-only list, so an ANS/SOL block is labelled once per question too; compact MC keys are unaffected.
...
- **Dashboard counts whole questions.** `hierarchy.paginate_by_root` slices the sorted result by root: a lettered tree counts as 1, a range stem counts as one per matched leaf (Q16 and Q17 are two questions that stay on the same page), standalones count as 1. `total_parts` is shown next to it when it differs. Range headers say `N questions share this data`. See [dashboard.md](dashboard.md).
- **Dashboard tree view.** A split question renders as one `.hierarchy-group`: a header card (root QID, part count, shared background shown **once**, `Whole` toggle when the root keeps a WHOLE archive) followed by `.hierarchy-children` rows built by `filter_questions` as `group.rows = [{kind: 'substem'|'leaf', card}]`. Intermediate stems (e.g. `Q1c`, the shared intro of `(c)(i)`/`(c)(ii)`) are emitted once, right before their first part, via `ancestors(leaf)` minus the root; every card carries `depth` (root 0, its parts 1, sub-parts 2) and the CSS indents with `--depth`. Part rows are compact (label `(a)` with the full QID only as a tooltip — the QID is printed once on the header — tags, own QUE only, icon actions) and never repeat the background. Range-stem leaves have no `part` (they are independent paper numbers, not `(a)`/`(b)`), so that letter is omitted — do not render `(None)`. A substem row has an aggregate `stem-checkbox` over its sub-parts and an Explain button that sends the intro plus every sub-part. `stem_preview` on a leaf is rendered only when the leaf is shown **outside** its group (standalone `leaf_card`, e.g. `ids`/`qids` overrides).
- **Split tool** is IMG QUE only. Requires a `stem` box plus ≥1 part label. Range stems cannot be split here. The page mounts shared `OQBBboxEditor` on the stitch (move/resize handles, magnifier, crop thumbnails/preview, Add-box draw mode, Delete-key parity with PDF import). Auto-detect **stitches** every QUE image for the version (reconstructed part crops, multi-page WHOLE, …) then runs PDF import **pass 2** once on that stitch (`detect_parts` + `full_width_child_box`, one retry on `LLMError`, SSE). A single-image QUE is unchanged. Do not run pass 1 (`detect_page`) unless **Image is a full exam page** is ticked. On a root, commit snapshots the staged full PNG as `WHOLE` (if that slot is empty) before cropping QUE. Commit payload is still `{token, copy_tags, boxes:[{label, box}]}`.
...
- **WHOLE** is never planned into papers, slides, Explain or auto-tag. Upload (Assets tab) is IMG-only on roots. Ingest skips WHOLE on a part-path or range stem. Where it is **visible** (all read-only, by asset id / root id):
  | Surface | What | Manage? |
  |---|---|---|
  | Edit modal → Assets (root only) | "Whole question archive" strip per version | yes: upload / reorder / delete |
  | Dashboard group header | `Whole` button swaps the shared-background preview for the archive images (`stem.whole_asset_ids`) | no (Edit button opens the root modal) |
  | Present (`viewer.html`) | `Whole` chip in the Question panel header (only when `question.whole_root_id`); shows `#wholePanel` via `viewer_asset/<root>/WHOLE` and hides background + part while on | no |
  | Question Management | archive icon next to the asset count (click = full-size preview); `Has archive` filter (`has_whole=1`) | no |
