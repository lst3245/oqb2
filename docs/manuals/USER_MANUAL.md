...
- A question that has been split into parts shows as one **group**: a header (QID, part count, the shared background shown once) and, indented below it, one compact row per part — the part label `(a)` (hover it for the part's full QID), its tags, its own text, and icon buttons (Answer, Solution, Explain, Edit). Questions that only share a data block (no `(a)`/`(b)`) have no part letter on those rows. A part that has its own sub-parts, e.g. `(c)` with `(i)` and `(ii)`, appears as a **shared intro** row with the `(i)`/`(ii)` rows indented under it. Ticking the header ticks every part of that question (including parts on other pages of the result); ticking a shared-intro row ticks all its sub-parts; ticking a part row selects that part only. When only some parts are ticked the header shows a dash (partially selected); clicking it then selects the rest. The background is always added automatically when you generate or view, so the selection count is simply the number of parts. **Select All** ticks every part and therefore every header too. The chevron on the header collapses the parts. If the question was imported in parts mode, the header also shows a **Whole** button: click it to see the original unsplit scan of the whole question in place of the shared background (click the image to enlarge; click **Whole** again to go back). This archive image is for reference only — papers and Present always use the background + parts.
- **Counts and pages are whole questions.** A split question (parts `(a)` `(b)`) counts once. Questions that only share a data block (e.g. DSE Econ “Questions 16 and 17”) count separately and stay together on the same page under a header that says they share this data. All matched parts of a split question always appear together. The result line reads e.g. `12 questions · 47 parts` when the two differ; **Select All (N questions, M parts)** still selects the parts. **Explain** on a header or shared-intro row sends the background together with every part below it, so the tutor walks through the whole question; Explain on a part sends that part with its background (and any earlier parts it depends on).
...
For a part of a split question, the Question panel first shows a collapsible **Shared background** block: the whole-question text, any intermediate intro (for example the text of `(c)` when you are viewing `(c)(ii)`), and — when the part is marked "uses earlier parts" — the earlier parts it refers to. This matches what the generated Word document prints before that part. If the question keeps an archived whole-question scan (imported in parts mode), a **Whole** button appears in the Question panel header: it swaps background + part for that single original image (zoom applies); click again to return. It stays on while you step through parts of the same question.
...
| Option | Description |
|---|---|
| Show QID on questions | Prints the QID (e.g. `MATC_DSE_2024_P1_Q5`) above each question — **once per question**. For a split question the line sits above the shared background (or above part (a) if there is no background); parts `(a)`, `(b)`, … are not re-labelled, since the image already shows the letter. |
| Show QID on answers | Same, but for the answer section (again once per question, above the first part's answer) |
| Show correct % | Appends the correct percentage: `MATC_DSE_2024_P1_Q5 [75%]` |
| Generate sequential question number | Adds runtime `1.`, `2.`, `3.` numbers to this generated paper; this is separate from the real-paper Question Number in the QID. Enabled by default for a new generation; regeneration and presets preserve their saved choice. Parts of the same long question share one number, printed once at the top of the question; a shared MC preamble does not consume a number. |
