...
- **Counts and pages are whole questions.** A split question counts once, however many parts matched your filter, and all of its matched parts always appear together on the same page. The result line reads e.g. `12 questions · 47 parts` when the two differ; **Select All (N questions, M parts)** still selects the parts. **Explain** on a header or shared-intro row sends the background together with every part below it, so the tutor walks through the whole question; Explain on a part sends that part with its background (and any earlier parts it depends on).
