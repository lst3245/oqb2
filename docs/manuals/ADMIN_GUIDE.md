...
### Split into parts
From Edit → Details, **Split into parts** opens a crop page for an **IMG** question that is not already a stem (Markdown/Word QUE is refused). Draw a **stem** box for the shared background and one box per part (`a`, `b`, `ci`, …), or click **Auto-detect parts** to let the vision model propose boxes, then adjust. If this question has several images (for example after Combine stacked the old parts), they are joined into one picture first, then Auto-detect runs the same **Detect parts** step as PDF import. A question that is already one image is detected as-is. Only tick **Image is a full exam page** when the picture still contains other questions or a large margin. Grey = stem, amber = part. The same boxes are applied to every version that has an image. Optionally copy this question's tags onto every part (the stem is then cleared). On a root question, Split also keeps a **whole-question archive** (the unsplit image) if one is not already stored, so Combine can restore it later.
...
2. **Bounding boxes** — optionally **Run LLM detection**, or draw/edit boxes on each page (drag, resize, Add box, per-page Re-run). For long structured papers tick **Split questions into parts** (starts on if the subject’s default is on): after numbered questions are found, a second pass cuts the shared background (grey) and lettered parts (amber). You can also click **Detect parts**, or per-question **Re-split** / **Unsplit**. Q# can be `3`, `3a`, `3ci`, or `23-24`; a part with its own sub-parts (`4d` above `4di`, `4dii`) is simply labelled that way and turns grey automatically — the grey/amber role is worked out from the labels you type, never set by hand. Every row has a **+** button that adds the next part directly beneath it (`4a → 4b`, `4di → 4dii`), the **Delete** key removes the highlighted box, and the link icon marks a part as **uses earlier parts** (see below). Set detection method, custom prompt, uniform width, and debug here.
