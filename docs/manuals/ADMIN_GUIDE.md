...
### Split into parts
From Edit → Details, **Split into parts** opens a crop page for an **IMG** question that is not already a stem (Markdown/Word QUE is refused). Draw a **stem** box for the shared background and one box per part (`a`, `b`, `ci`, …), or click **Auto-detect parts** to let the vision model propose boxes, then adjust. The editor is the same overlay used in PDF Batch Import: drag to move, eight resize handles, a live magnifier while dragging, crop thumbnails and a full-size preview, an **Add box** button (draw mode), and **Delete** to remove the highlighted box. If this question has several images (for example after Combine stacked the old parts), they are joined into one picture first, then Auto-detect runs the same **Detect parts** step as PDF import. A question that is already one image is detected as-is. Only tick **Image is a full exam page** when the picture still contains other questions or a large margin. Grey = stem, amber = part. The same boxes are applied to every version that has an image. Optionally copy this question's tags onto every part (the stem is then cleared). On a root question, Split also keeps a **whole-question archive** (the unsplit image) if one is not already stored, so Combine can restore it later.
...
1. **Setup** — choose Exam paper or Generic extraction, enter the paper name (exam), upload or pick question/solution PDFs, set deskew/pre-process, optionally set **Pages** (1-based source PDF range such as `1-8` or `3,5,12-14`; blank loads every page), then **Load PDF**.
2. **Bounding boxes** — optionally **Run LLM detection**, or draw/edit boxes on each page (drag, resize handles, **Add box** draw mode, per-page Re-run). **Snap boxes to page frame** starts on: when a printed rectangle was found on the scan, boxes lock to that width (dashed green guide; drag the left/right rails to correct a page; each page has a **Frame** button for “No frame” / “Add frame”). **Uniform width per side** is a leftover option, off by default, and is ignored on framed pages while snap is on. For long structured papers tick **Split questions into parts** (starts on if the subject’s default is on): after numbered questions are found, a second pass cuts the shared background (grey) and lettered parts (amber). You can also click **Detect parts**, per-question **Re-split** / **Unsplit**, or a page's **Re-split page** button to redo the part split for every question on that page without starting over (a question that continues on another page is redone across all its pages so the link is kept). Re-run, Re-split and the AI agent all respect the green frame. While a Re-run or Re-split is in flight the clicked button shows a spinner and the other page actions grey out (the log still lists each model step). Q# can be `3`, `3a`, `3ci`, or `23-24`; a part with its own sub-parts (`4d` above `4di`, `4dii`) is simply labelled that way and turns grey automatically — the grey/amber role is worked out from the labels you type, never set by hand. Every row has a **+** button that adds the next part directly beneath it (`4a → 4b`, `4di → 4dii`), the **Delete** key removes the highlighted box, and the link icon marks a part as **uses earlier parts** (see below). Set detection method, custom prompt, and debug here.
   - **Run AI agent** (exam papers, after the PDF is loaded): instead of clicking detection buttons, click **Run AI agent** in this review step. The agent first reads the whole paper as thumbnails to build an outline (which pages hold questions, which numbers, which parts), then locates questions with those expectations, splits multi-part questions, re-draws its own boxes and asks the model to check them, and repairs what it can. Anything it could not settle appears in the **Needs attention** panel (red = a question or part it could not find, amber = something to double-check); click an entry to jump to the box. Review only those, then Import. Running the agent replaces any boxes already in the session. The run continues on the server if the browser tab drops; reopen the page or click **Run AI agent** to rejoin. Cost is capped by **AI agent: LLM call budget per run** in System Settings.
3. **Import** — exam: pick QUE/SOL versions, overwrite/trim options, **Import to database**; generic: **Download all as ZIP**. Parts flagged "uses earlier parts" are stored with that flag.
...
| PDF Import | `PDF_IMPORT_RASTER_WIDTH`, `PDF_IMPORT_RASTER_WORKERS`, `PDF_IMPORT_COORD_ORDER`, `PDF_IMPORT_DESKEW_DEFAULT`, `PDF_IMPORT_DEFAULT_METHOD`, `PDF_IMPORT_CROP_PAD_PCT`, `PDF_IMPORT_REFINE_GROW_PCT`, `PDF_IMPORT_ASSIST_PAD_PCT`, `PDF_IMPORT_TRIM_WHITE_DEFAULT`, `PDF_IMPORT_UNIFORM_WIDTH_DEFAULT`, `PDF_IMPORT_FRAME_SNAP_DEFAULT`, `PDF_IMPORT_FRAME_INSET_PCT`, `PDF_IMPORT_FRAME_NORMALISE_WIDTH` | Raster quality / parallelism, bounding-box axis order for the vision model, page-frame snap / inset / crop-width normalise, and the defaults pre-ticked in the PDF Batch Import wizard. |
