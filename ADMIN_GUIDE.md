...
- **AI Tools** — runs a configured LLM (local or cloud) over the selected questions. Two operations:
  ...
  **Per-question shortcut:** inside the edit-question modal — on **both** the Question Management page and the dashboard — each version/asset-type slot's **Markdown** area has a **Generate with AI** button (and a robot *regenerate* button when a Markdown asset already exists). It transcribes that version's own image(s) into Markdown via a chosen endpoint — a quick way to fill one slot without running a batch. These per-slot controls appear for any admin when AI Tools are enabled.
