...
4. **Redact / Highlight / Mark up** (optional) —
   - **Mark up** opens a fullscreen editor: draw redact boxes, highlights, text, or freehand pen strokes; move/resize/delete marks; **Copy to → all/selected pages** stamps a mark across the document.
   - **Find & Mark** searches for phrases and marks every match: engines are **Auto**, **Digital** (PDF text layer), **OCR** (scanned pages — needs Tesseract, see Notes), or **AI** (vision LLM with an editable instruction, e.g. "every signature"; reuses the PDF Import endpoints). Fuzzy matching with a threshold slider handles OCR noise; phrases broken across lines/pages still match. Results arrive as **pending** marks (dashed orange) — review and **Accept all**, or adjust/delete them in the editor first. Pending marks are never exported.
