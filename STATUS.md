...
| Item | State | Notes |
|---|---|---|
| Automated tests | `[partial]` | 11 `unittest` modules in `tests/` (hierarchy incl. grouping/`paginate_by_root`/split-box/derived roles, combine/WHOLE filenames/`whole_source_crops`, PDF import plan labels + page-range + split-pipeline helpers, `test_page_frame` (`detect_page_frame` synthetics, `consolidate_frames`, `frame_x_extent`, `snap_boxes_to_frames`, `crop_page` `trim_axis`, `FrameCropper.is_snapped`), PDF agent pure logic, sorting, MC keys + single-heading-per-question, generate template UX, PDF tool split, AI prompts incl. agent parsers, LLM client). No DB-backed tests, no pytest, no CI. `tests/test_hierarchy.py`, `tests/test_question_combine.py`, `tests/test_pdf_import_plan.py` and `tests/test_page_frame.py` are pure (no `create_app()`). `tests/test_llm_client.py` calls `create_app()` and therefore touches the live DB. |
