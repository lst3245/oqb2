...
| Dashboard filter / search / multi-sort / pagination | `[done]` | `app/dashboard.py` | [modules/dashboard.md](docs/modules/dashboard.md) | Python-side sort with manual block order; leaves-only list grouped under stem headers; pages / `total` count **whole questions** (`paginate_by_root`, a root never straddles pages, `total_parts` shown when different); group header `Whole` toggle shows the WHOLE archive (view-only) |
...
| Document generation (.docx, answer modes, compact MC keys, split ZIP, styles) | `[done]` | `app/generator.py` | [modules/generator.md](docs/modules/generator.md) | Background thread; status polling; `hierarchy_mode` + `resolve_render_plan`; one `N. QID` heading per whole question (`_assign_headings`), parts unlabelled |
...
| Viewer / presentation mode | `[done]` | `app/generator.py`, `templates/viewer.html` | [modules/generator.md](docs/modules/generator.md) | Does not extend `base.html`. Slides are leaves; stem QUE in `#stemPanel`; `Whole` chip shows the root's WHOLE archive (`viewer_asset/<root>/WHOLE`) |
...
| Question Management (list, filters, Status filter, select-all-matching) | `[done]` | `app/admin.py` | [modules/admin-questions.md](docs/modules/admin-questions.md) | Create/rename/delete understand stems/parts; Part column + Tree filter (`all`/`roots`/`leaves`); WHOLE archive icon (quick preview) + `Has archive` filter; bulk Combine / Split |
...
| Subject AI tuning (per-subject Auto Tag instructions append/replace, taxonomy hints, prompt preview, evaluate vs existing tags, teacher-correction log + report + feedback into prompt) | `[done]` | `app/subject_ai.py`, `app/subject_ai_service.py`, `app/ai_tools.build_tag_prompt` | [modules/subject-ai.md](docs/modules/subject-ai.md) | Subject-admin tier; format contract stays server-controlled. Hidden taxonomy nodes no longer sent to the model. `suggest_tags` returns `confidence` / `reasons`. Tests `tests/test_subject_ai.py`. ADR-012 |
...
| Question hierarchy (stem / parts) | `[done]` | `app/hierarchy.py`, `app/question_split.py`, `app/question_combine.py`, `app/models.py` | [modules/question-hierarchy.md](docs/modules/question-hierarchy.md) | Schema, grammar, ingest/create/rename/delete, dashboard grouping, generator/viewer render plan, admin tree UI, IMG Split (stitch multi-image QUE then SSE pass-2 `detect_parts` once; crop UI is shared `OQBBboxEditor`) + Combine (restore WHOLE / reconstruct), PDF two-pass, AI ancestor images, `derive_roles` / `next_part_label`, `needs_prev_parts` (earlier siblings as background). `WHOLE` asset_type is root-only unsplit original, viewable (read-only) from dashboard header / Present / Question Management, managed only in the edit modal. Dashboard pages and docx headings are per whole question. ADR-009 |
...
| PDF Batch Import | `/admin/pdf-import` | `admin_pdf_import.html` (`OQBBboxEditor` per page) | Admin dropdown | `[done]` |
...
| Item | State | Notes |
|---|---|---|
| Automated tests | `[partial]` | 13 `unittest` modules in `tests/` (hierarchy incl. grouping/`paginate_by_root`/split-box/derived roles, combine/WHOLE filenames/`whole_source_crops`, PDF import plan labels + page-range + split-pipeline helpers, `test_page_frame` (`detect_page_frame` synthetics, `consolidate_frames`, `frame_x_extent`, `snap_boxes_to_frames`, `crop_page` `trim_axis`, `FrameCropper.is_snapped`), PDF agent pure logic, sorting, MC keys + single-heading-per-question, generate template UX, PDF tool split, AI prompts incl. agent parsers, subject AI, LLM client, `test_doc_thumbnails` crop box / footer-island bbox / `strip_docx_header_footer_refs`). No DB-backed tests, no pytest, no CI. `tests/test_hierarchy.py`, `tests/test_question_combine.py`, `tests/test_pdf_import_plan.py`, `tests/test_page_frame.py` and `tests/test_doc_thumbnails.py` are pure (no `create_app()`). `tests/test_llm_client.py` calls `create_app()` and therefore touches the live DB. |
