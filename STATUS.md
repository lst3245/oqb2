...
| Item | State | Notes |
|---|---|---|
| Automated tests | `[partial]` | 9 `unittest` modules in `tests/` (hierarchy incl. grouping/split-box/derived roles, PDF import plan labels, PDF agent pure logic, sorting, MC keys, generate template UX, PDF tool split, AI prompts incl. agent parsers, LLM client). No DB-backed tests, no pytest, no CI. `tests/test_hierarchy.py` and `tests/test_pdf_import_plan.py` are pure (no `create_app()`). `tests/test_llm_client.py` calls `create_app()` and therefore touches the live DB. |
