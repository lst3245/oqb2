...
- **Toolbox PDF Tool: multi-page drag, bulk reorder, contrast/sharpen, and Undo.** Several assembly-UX refinements: (1) dragging a multi-page selection now **lifts the whole group** — the other selected pages dim and the drag image shows a **count badge** — so it's clear they all travel together. (2) A new **Reorder** toolbar dropdown bulk-rearranges the selection (or all pages if nothing is selected): **Reverse**, **Interleave** (weave two halves — e.g. recombine separately-scanned fronts and backs), **Interleave (reverse 2nd half)** for reversed duplex back-side stacks, and **Deinterleave** (split alternating pages into two consecutive groups). (3) **Contrast ±**, **Sharpen ±**, and **Grayscale** toggle buttons were added to the operations toolbar (previously only reachable in the "Process & add" step). (4) An **Undo** button (and **Ctrl+Z**) reverts the last action — add, edit, reorder/drag, delete, copy/cut/paste — restored via the new `/toolbox/pdf/set-pages` endpoint that mirrors the client working set back to the server session. (5) The preview area is wrapped in a taller **canvas** so there's ample empty space to start a rubber-band selection.
