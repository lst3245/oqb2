...
- **Live magnifier while editing boxes.** Dragging, drawing, or resizing a PDF Import review box now shows a floating zoom window based on the original page image pixels, with crosshairs at the cursor/handle and the active crop rectangle overlaid. This makes it much easier to place crop edges precisely on scanned pages.
