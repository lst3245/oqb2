...
- **Smart Import (unified ingestion + heuristic folder import).** The old Question Ingestion page is replaced by one **Smart Import** tool (`/admin/import`, navbar + Admin index) with two modes:
  ...
  - **Upload from computer.** Besides picking a server folder, you can **upload a whole folder from your computer** (`webkitdirectory`); files are staged on the server and matched the same way.
