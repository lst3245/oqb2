...
- **Markup: normalize imported image resolution.** Imported images are now scaled to a consistent on-canvas size (longest edge → 1600 world units) regardless of source resolution. Previously a low-resolution image occupied a tiny world space, so fitting it forced a large zoom-in where each 1px pen-size step became a huge visible change; high-resolution scans had the opposite problem. Normalization keeps the fit zoom level and pen-size scale consistent across imports, while export still renders from the full-resolution source so output quality is preserved.
